...
export default function DeckCardGallery({ entries, onOpen, onQuantityChange, onRemove, t, label, isUpdating }) {
  if (entries.length === 0) {
      ...
      {entries.map((entry, index) => (
        <article key={entry.id} className="group relative overflow-hidden rounded-xl border border-border bg-bg-card shadow-sm">
          ...
          <div className="flex items-center justify-between gap-1 border-t border-border bg-bg-surface/95 px-1.5 py-1.5">
            <button type="button" className="rounded-md p-1.5 text-text-muted hover:bg-bg-elevated hover:text-text-primary disabled:opacity-40" disabled={isUpdating || entry.required_quantity <= 1} onClick={() => onQuantityChange(entry, entry.required_quantity - 1)} aria-label={label('decks.decreaseQuantity', { name: entry.card?.name || entry.card_id })}><Minus size={14} /></button>
            ...
            <button type="button" className="rounded-md p-1.5 text-text-muted hover:bg-bg-elevated hover:text-text-primary disabled:opacity-40" disabled={isUpdating} onClick={() => onQuantityChange(entry, entry.required_quantity + 1)} aria-label={label('decks.increaseQuantity', { name: entry.card?.name || entry.card_id })}><Plus size={14} /></button>
            <button type="button" className="rounded-md p-1.5 text-text-muted hover:bg-brand-red/15 hover:text-brand-red disabled:opacity-40" disabled={isUpdating} onClick={() => onRemove(entry)} aria-label={label('decks.removeCard', { name: entry.card?.name || entry.card_id })}><Trash2 size={14} /></button>
          </div>
        </article>
      ))}
    </div>
