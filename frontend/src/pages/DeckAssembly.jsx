...
export default function DeckAssembly() {
  ...
  return (
    <div className="deck-assembly space-y-4 pb-8">
      ...
      <DeckCardViewer entries={entries} activeIndex={viewerIndex} onClose={() => setViewerIndex(null)} onPrevious={() => setViewerIndex(index => previousDeckCardIndex(index, entries.length))} onNext={() => setViewerIndex(index => nextDeckCardIndex(index, entries.length))} t={t} label={(key, values) => Object.entries(values).reduce((text, [name, value]) => text.replace(`{${name}}`, value), t(key))} />
    </div>
  )
}
