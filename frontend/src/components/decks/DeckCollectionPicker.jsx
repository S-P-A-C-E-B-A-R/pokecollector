...
export default function DeckCollectionPicker({ collection, entries, onAdd, optimisticAddQuantities, pendingCardIds, t, label }) {
  ...
  return (
    <section className="card space-y-3">
      <div className="flex items-center justify-between gap-2"><h2 className="font-semibold text-text-primary">{t('decks.addCards')}</h2>{hasFilters && <button type="button" className="btn-ghost px-2 text-xs" onClick={reset}><RotateCcw size={13} /> {t('decks.clearFilters')}</button>}</div>
      ...
      <Modal isOpen={Boolean(selected) && !isDesktop} onClose={() => setSelectedId(null)} title={selected?.card.name} size="lg">{preview}</Modal>
    </section>
  )
}
